...
bot.once('ready', function (evt) {
	...
	var ddcStart = new Date(DDCStartStr); // Define as 2019 Jul 14th, 7pm, in UTC (the "Z"). "T" is separator between date and time.
	...
	if (tDiff < 0) { // DDC is ongoing. Find current event.
		...
		for (i = 0; i < DDCEvents.length; i++) {
			...
			if (daysNum >= DDCEvents[i].day && daysNum < DDCEvents[i].day+2) { // Is within time frame of event i.
				//console.log("event found for startup! "+daysNum +">="+ DDCEvents[i].day +"&&"+ daysNum +"<"+ (DDCEvents[i].day+2) + " and i="+i);
				...
				console.log("event found for startup! Announcing.");
...
bot.on('message', message => {
	...
	if (message.content.toLowerCase() === ">ddc" || message.content.toLowerCase() === ">ddctime"){
	...
	if (message.content.toLowerCase() === ">ddcrules" || message.content.toLowerCase() === ">ddchelp" || message.content.toLowerCase() === ">ddcschedule"){ 
...
function Announce() { // Ended up being pretty much a copy/paste of startup re-announcement.
	...
	var tDiff = ddcStart.getTime() - curTime.getTime() - 100; // Find time until ddcStart since curTime. Minus 100 extra ms to try to cover any timing weirdness that may exist.
	console.log("tDiff: " + tDiff);
	...
	if (tDiff < 0) { // DDC is ongoing. Find current event.
		...
		tDiff = -tDiff; // Make positive just for clarity.
		var daysNum = tDiff / msInDay; // Get days since DDC start, with decimal.
		console.log("daysNum: " + daysNum + ", DDCEvents[i].title: "+DDCEvents[i].title + ", DDCEvents[i].day: "+DDCEvents[i].day);
		for (i = 0; i < DDCEvents.length; i++) {
			// Need to take into account Saturdays as "sabbaths" from 19:00 to 19:00 Sunday. Alternatively, just look 48 hours ahead, to the end of the event.
			if (daysNum >= DDCEvents[i].day && daysNum < DDCEvents[i].day+2) { // Is within time frame of event i.
				console.log("event found for Announce! ("+i+") Announcing.");
				var timeLeft = new Date((DDCEvents[i].day+2 - daysNum) * msInDay);
				var lastEvent = "";
				...
				if (i != DDCEvents.length-1) {
					if (!(daysNum >= DDCEvents[i+1].day && daysNum < DDCEvents[i+1].day+2)) { // Not actually just part of next event.
...
function AskEvent(ch) { // User asks the current event and time left. ch is channel. Mostly copy and paste of startup.
	var curTime = new Date(); // Automatically sets to current time.
	...
	if (tDiff < 0) { // DDC is ongoing. Find current event.
		...
		for (i = 0; i < DDCEvents.length; i++) {
			// Need to take into account Saturdays as "sabbaths" from 19:00 to 19:00 Sunday. Alternatively, just look 48 hours ahead, to the end of the event.
			if (daysNum >= DDCEvents[i].day && daysNum < DDCEvents[i].day+2) { // Is within time frame of event i.
				console.log("event found for Ask! Responding.");
				var timeLeft = new Date((DDCEvents[i].day+2 - daysNum) * msInDay);
				var lastEvent = "";
				if (i == DDCEvents.length-1) {lastEvent = " This is the **last** DDC challenge of Season 1!";}
...
function SetTimers(dayNumID, timeToNextInMS, updateOneTimer=false) {
	...
	if (dayNumID == 0) {
		timer1 = setTimeout(Announce, timeToNextInMS);
		timer2 = setTimeout(Announce, timeToNextInMS + 2*msInDay);
		timer3 = setTimeout(Announce, timeToNextInMS + 4*msInDay);
	}
	else if (dayNumID == 2) {
		timer2 = setTimeout(Announce, timeToNextInMS);
		timer3 = setTimeout(Announce, timeToNextInMS + 2*msInDay);
		timer1 = setTimeout(Announce, timeToNextInMS + 5*msInDay); // Skip 1 for WWG sabbath.
	}
	else if (dayNumID == 4) {
		timer3 = setTimeout(Announce, timeToNextInMS);
		timer1 = setTimeout(Announce, timeToNextInMS + 3*msInDay); // Skip 1 for WWG sabbath.
		timer2 = setTimeout(Announce, timeToNextInMS + 5*msInDay);
	}
